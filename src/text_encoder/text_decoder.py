...
class CTCArgmaxDecoder(CTCBaseDecoder):
    def decode(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        return pad_sequence(
            [torch.tensor([x for x, _ in itertools.groupby(sample)]) for sample in ids],
            padding_item=0,
        )
...
class CTCBeamSearchDecoder(CTCBaseDecoder):
    ...
    def decode(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        if logits.ndim == 3:
            return pad_sequence(
                [self.decode(logits_sinlge_batch) for logits_sinlge_batch in logits],
                padding_item=0,
            )

        current_beams = [(tuple(), (0.0, -INF))]

        for t in range(logits.shape[0]):
            next_beams = defaultdict(
                lambda: (-INF, -INF)
            )  # so if no such prefix exists then all probs are 0
            for c in range(logits.shape[1]):
                ...
                for prefix, (p_bl, p_n_bl) in current_beams:
                    if c == self.blank:
                        ...
                        next_p_bl = self.stable_log_prob_sum(
                            next_p_bl, p_bl + p, p_n_bl + p
                        )
