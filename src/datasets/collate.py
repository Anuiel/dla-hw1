...
def pad_sequence(data: list[torch.Tensor], padding_item: float | int = 0) -> torch.Tensor:
    ...
    for item in data:
        ...
        padded_item = torch.nn.functional.pad(item, (0, time_padding), mode="constant", value=padding_item)
...
def collate_fn(dataset_items: list[dict]):
    ...
    spectrogram = pad_sequence([item["spectrogram"].squeeze(0) for item in dataset_items], padding_item=math.log(1e-6))
    ...
    text_encoded = pad_sequence([item["text_encoded"].squeeze(0) for item in dataset_items], padding_item=0)
    return {
